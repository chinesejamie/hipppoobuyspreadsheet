    ...
    if (productsSection) {
      ...
      overlay.innerHTML = `
        <div class="text-center">
          <div class="inline-block animate-spin rounded-full h-16 w-16 border-4 border-[#FF186B] border-t-transparent mb-4"></div>
  ...
  return (
    <div className="flex flex-col sm:flex-row items-center justify-between gap-3 sm:gap-4 bg-white/80 backdrop-blur-lg rounded-xl sm:rounded-2xl shadow-lg border border-gray-200 p-3 sm:p-4 md:p-6">
      <div className="text-xs sm:text-sm text-gray-600">
        Showing <span className="font-bold text-[#FF186B]">{productsCount}</span> products on page <span className="font-bold">{currentPage}</span>
      </div>
      ...
      <div className="flex gap-2">
        ...
        <div className={`flex items-center px-3 sm:px-6 py-3 bg-gradient-to-r from-purple-100 to-pink-100 rounded-xl border-2 border-[#FF186B]/20 shadow-md transition-all ${clickedButton ? 'animate-pulse' : ''}`}>
          {clickedButton ? (
            <Loader2 className="w-4 h-4 sm:w-5 sm:h-5 animate-spin text-[#FF186B] mr-2" />
          ) : null}
